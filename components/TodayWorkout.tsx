    ...
    <View className='bg-white rounded-[10] p-4'
      ...
      <Text className='font-bold text-sm text-[#323232]'>PRÓXIMO TREINO</Text>
        {data.map((allWorkout, index) => (
            <Text key={index}
                className='text-[40px] font-bold text-[#323232] mb-2'
            >
        ...
        <View className='flex-row items-center gap-2 mb-6'>
            ...
            {data.map((allWorkout, index) => (
                <Text key={index}
                    className='text-md font-bold text-[#323232]'
                >
        ...
        <TouchableOpacity className='bg-[#323232] mx-auto w-full py-4 rounded-[8]' onPress={() => router.push('/(auth)/home')}>
